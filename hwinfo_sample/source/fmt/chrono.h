...
inline void do_write(buffer<Char>& buf, const std::tm& time,
  ...
  if (end.failed()) log_error("failed to format time");
}
...
inline std::tm localtime(std::time_t time) {
  ...
  if (!lt.run()) log_error("time_t value out of range");
  return lt.tm_;
}
...
inline std::tm gmtime(std::time_t time) {
  ...
  dispatcher gt(time);
  // Too big time values may be unsupported.
  if (!gt.run()) log_error("time_t value out of range");
  return gt.tm_;
}
...
FMT_CONSTEXPR const Char* parse_chrono_format(const Char* begin,
  ...
  while (ptr != end) {
    ...
    ++ptr;  // consume '%'
    if (ptr == end) log_error("invaild format");
    c = *ptr++;
    switch (c) {
    case '%':
    ...
    case 'E': {
      if (ptr == end) log_error("invalid format");
      c = *ptr++;
      switch (c) {
      case 'Y':
...
struct tm_format_checker : null_chrono_spec_handler<tm_format_checker> {
  void unsupported() { log_error("no format"); }
...
struct chrono_format_checker : null_chrono_spec_handler<chrono_format_checker> {
  FMT_NORETURN void unsupported() { log_error("no date"); throw std::runtime_error("no date"); }
...
To fmt_safe_duration_cast(std::chrono::duration<FromRep, FromPeriod> from) {
  ...
  if (ec) log_error("cannot format duration");
  return to;
}
#endif
...
struct formatter<std::chrono::duration<Rep, Period>, Char> {
 private:
  ...
  struct spec_handler {
    ...
    void on_error(const char* msg) { log_error(msg); }
