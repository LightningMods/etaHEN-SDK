...
#define ALIGN_SBL(x, a)		__ALIGN_KERNEL((x), (a))
...
int _sceSblAuthMgrSmFinalize( int authmgr_handle, int context_id);

// Verifies SELF header and initializes context for future operations
int _sceSblAuthMgrVerifyHeader( int authmgr_handle, uint64_t header_pa, uint64_t header_len);

// Decrypts a SELF segment
int _sceSblAuthMgrSmLoadSelfSegment( int authmgr_handle, int service_id, uint64_t chunk_table_pa, uint32_t segment_index);
